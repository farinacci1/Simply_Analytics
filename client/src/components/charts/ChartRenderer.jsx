...
import { VerticalBarChartWrapper, HorizontalBarChartWrapper, DivergingBarChartWrapper, LineChartWrapper, AreaChartWrapper, PieChartWrapper, TreemapChartWrapper, IcicleChartWrapper, SankeyChartWrapper, FunnelChartWrapper, WaterfallChartWrapper, ScatterChartWrapper, BoxPlotChartWrapper, MetricCard, DataTable, ChoroplethChartWrapper, HexbinChartWrapper, HeatmapChartWrapper, GaugeChartWrapper, RadarChartWrapper, HistogramChartWrapper, getColorArray } from './index';
  ...
  const getChartSvg = () => {
    ...
    switch (type?.toLowerCase()) {
        ...
        return (
          <svg {...baseProps} viewBox="0 0 120 80">
            {[0,1,2,3].map(r => [0,1,2,3,4].map(c => (
              <rect key={`${r}${c}`} x={22+c*19} y={10+r*16} width="16" height="13" rx="2"
                fill={color} opacity={0.15 + ((r*5+c) % 7) * 0.1} />
            )))}
          </svg>
        );
      case 'gauge':
        return (
          <svg {...baseProps} viewBox="0 0 120 80">
            <path d="M 25 65 A 35 35 0 0 1 95 65" fill="none" stroke={mutedColor} strokeWidth="8" opacity="0.2" strokeLinecap="round" />
            <path d="M 25 65 A 35 35 0 0 1 80 25" fill="none" stroke={color} strokeWidth="8" opacity="0.5" strokeLinecap="round" />
            <circle cx="60" cy="65" r="4" fill={color} opacity="0.6" />
            <line x1="60" y1="65" x2="78" y2="30" stroke={color} strokeWidth="2" opacity="0.5" />
          </svg>
        );
      case 'radar':
        return (
          <svg {...baseProps} viewBox="0 0 120 80">
            <polygon points="60,12 95,32 85,68 35,68 25,32" fill="none" stroke={mutedColor} strokeWidth="0.5" opacity="0.3" />
            <polygon points="60,25 82,37 76,60 44,60 38,37" fill="none" stroke={mutedColor} strokeWidth="0.5" opacity="0.3" />
            <polygon points="60,18 90,34 80,64 40,64 30,34" fill={color} fillOpacity="0.2" stroke={color} strokeWidth="1.5" opacity="0.6" />
            {[[60,18],[90,34],[80,64],[40,64],[30,34]].map(([x,y],i) => (
      ...
      case 'histogram':
        return (
          <svg {...baseProps} viewBox="0 0 120 80">
            <line x1="20" y1="70" x2="110" y2="70" stroke={mutedColor} strokeWidth="1" opacity="0.3" />
            <line x1="20" y1="10" x2="20" y2="70" stroke={mutedColor} strokeWidth="1" opacity="0.3" />
            {[[24,55],[36,40],[48,22],[60,18],[72,30],[84,48],[96,58]].map(([x,y],i) => (
              <rect key={i} x={x} y={y} width="10" height={70-y} rx="1" fill={color} opacity={0.3 + i*0.05} />
            ))}
          </svg>
        );
      case 'choropleth':
      case 'hexbin':
        return (
          <svg {...baseProps} viewBox="0 0 120 80">
            <ellipse cx="60" cy="40" rx="45" ry="30" fill="none" stroke={mutedColor} strokeWidth="0.5" opacity="0.3" />
            <path d="M 30 30 L 50 25 L 60 35 L 55 45 L 35 40 Z" fill={color} opacity="0.3" />
            <path d="M 62 22 L 80 20 L 90 35 L 85 45 L 65 40 Z" fill={color} opacity="0.4" />
            <path d="M 40 48 L 55 47 L 60 55 L 50 60 L 38 55 Z" fill={color} opacity="0.25" />
            <path d="M 65 45 L 85 47 L 80 58 L 62 55 Z" fill={color} opacity="0.35" />
          </svg>
        );
      default:
        return (
...
const resolveChart = (type, data, config, query, fieldAggregations) => {
  switch (type) {
    ...
    case 'heatmap':
      return <HeatmapChartWrapper data={data} config={config} query={query} />;
    case 'gauge':
      return <GaugeChartWrapper data={data} config={config} query={query} />;
    case 'radar':
      return <RadarChartWrapper data={data} config={config} query={query} />;
    case 'histogram':
      return <HistogramChartWrapper data={data} config={config} query={query} />;
    case 'choropleth':
      return <ChoroplethChartWrapper data={data} config={config} query={query} />;
    case 'hexbin':
      return <HexbinChartWrapper data={data} config={config} query={query} />;
    case 'funnel':
